...
const parts: Part[] = [
  { number: 'I', title: 'The Builder Reality Check', desc: 'Why you haven\'t built anything yet, the product gap, the startup myth, and how this book changes everything.', wide: true },
  { number: 'II', title: 'The Builder Mindset', desc: 'Rewire how you think about building. Break the tutorial trap and perfectionism cycle.' },
  { number: 'III', title: 'Find & Validate the Problem', desc: 'Identify yourself as a builder, read markets, think problem-first, and validate before building.', tall: true },
  { number: 'IV', title: 'Design the Solution', desc: 'Think in systems, research your domain, and design before you write a single line of code.' },
  { number: 'V', title: 'Build It', desc: 'Build an MVP, make smart stack decisions, and debug like a thinker, not a guesser.' },
  { number: 'VI', title: 'Deploy & Show', desc: 'Ship your first version, get your first real user, and launch on channels that work.', wide: true },
  { number: 'VII', title: 'Listen & Iterate', desc: 'Collect real feedback, know when to pivot, and iterate without breaking things.' },
  { number: 'VIII', title: 'Build in Public', desc: 'Build on GitHub, grow on Twitter/LinkedIn, find your community, and play the long game.' },
];
  ...
  return (
    <section className={styles.hero}>
      <div className={styles.heroContainer}>
        <h1 className={styles.heroTitle}>Ship It.</h1>
        <p className={styles.heroSubtitle}>{siteConfig.tagline}</p>
        <div className={styles.statsBar}>
          <div className={styles.stat}>
            <div className={styles.statValue}>90%</div>
            <div className={styles.statLabel}>Side projects never launch</div>
          </div>
          <div className={styles.stat}>
            <div className={styles.statValue}>70%</div>
            <div className={styles.statLabel}>Startups fail — no market need</div>
          </div>
          <div className={styles.stat}>
            <div className={styles.statValue}>85%</div>
            <div className={styles.statLabel}>Developers never ship a product</div>
          </div>
        </div>
        <Link className={styles.cta} to="/introduction/">
          Start Reading
        ...
        <div className={styles.partsGrid}>
          {parts.map((part, idx) => (
            <Link
              key={idx}
              to={partLinks[idx] || '/'}
              className={`${styles.partLink} ${part.wide ? styles.partWide : ''} ${part.tall ? styles.partTall : ''}`}
            >
              <div className={styles.partCard}>
                <div className={styles.partNumber}>{part.number}</div>
                <h3 className={styles.partName}>{part.title}</h3>
      ...
      <div className={styles.authorInner}>
        <div className={styles.authorLayout}>
          <div className={styles.authorLeft}>
            <h2 className={styles.authorName}>Muhammad<br />Hamid Raza</h2>
            <p className={styles.authorRole}>Builder &amp; Author</p>
          </div>
          <div className={styles.authorRight}>
            <blockquote className={styles.pullQuote}>
              This book is not written from theory alone, but from building real products, launching startups, failing, learning, and shipping again.
            </blockquote>
            ...
            <div className={styles.authorSocials}>
              ...
              <a href="https://muhammadhamidraza.vercel.app/" target="_blank" rel="noopener noreferrer" className={styles.socialLink} aria-label="Website">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" width="16" height="16"><circle cx="12" cy="12" r="9" fill="none"/><ellipse cx="12" cy="12" rx="9" ry="3.5"/><line x1="3" y1="12" x2="21" y2="12"/><path d="M12 3c-2 1.5-3.5 4-3.5 9s1.5 7.5 3.5 9"/><path d="M12 3c2 1.5 3.5 4 3.5 9s-1.5 7.5-3.5 9"/></svg>
                Website
              </a>
            </div>
          </div>
